...
export const velocityRouter = router({
  ...
  getWeeklyVelocity: publicProcedure
    ...
    .query(({ ctx, input }) => {
      ...
      // Query all tasks completed since the oldest week start
      const completedTasks = ctx.db
        .select()
        .from(tasks)
        .where(and(eq(tasks.status, 'done'), gte(tasks.updated_at, oldestWeekStart)))
        .all()
  ...
  getDailyVelocity: publicProcedure
    ...
    .query(({ ctx, input }) => {
      ...
      // Query all tasks completed since the oldest day start
      const completedTasks = ctx.db
        .select()
        .from(tasks)
        .where(and(eq(tasks.status, 'done'), gte(tasks.updated_at, oldestDayStart)))
        .all()
